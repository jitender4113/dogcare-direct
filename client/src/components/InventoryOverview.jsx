...
function InventoryOverview({ categorySummary, items }) {
    return (
        <div className="mt-8 rounded-[28px] bg-white p-7 shadow-sm">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-extrabold text-[#193024]">Inventory Overview</h2>
                    <p className="mt-1 text-sm text-[#7D887F]">Current stock levels by category</p>
                </div>
            ...
            <div className="mt-7 space-y-6">
                {categorySummary.map((summary) => {
                    ...
                    const percentage = clampPct(summary.stockPercentage);

                    return (
                        <div key={category}>
                            <div className="mb-2 flex items-center justify-between">
                                <p className="text-sm font-bold text-[#193024]">{category}</p>
                                <p className="text-sm font-semibold text-[#7D887F]">
                                    {summary.totalQuantity} / {summary.totalRequired}
                                </p>
                            </div>
                            ...
                            <p className="mt-1 text-xs text-[#9AA39C]">{percentage}% stocked</p>
                            ...
                            <div className="mt-3 space-y-2">
                                {items
                                    ...
                                    .map((item) => (
                                        ...
                                        >
                                            <p className="text-sm font-medium text-[#193024]">{item.name}</p>
